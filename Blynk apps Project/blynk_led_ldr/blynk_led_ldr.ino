...
//initialize the pin for LED
int ledPinG = 14;   //D5
int ledPinY = 12;   //D6
int ledPinR = 13;   //D7

//initialize to use the LED display in the blynk apps
WidgetLED ledG(V1);
WidgetLED ledY(V2);
WidgetLED ledR(V3);

//analog pin for LDR sensor to be read
int ldrPin = A0;
...
void loop() {
  ...
  if (ldrVal<= 125){
    ...
    //codes for control the LED display in blynk apps
    ledG.on();
    ledY.on();
    ledR.off();
  }
